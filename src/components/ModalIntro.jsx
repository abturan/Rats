// src/components/ModalIntro.jsx
import React from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import ActionButton from './ui/ActionButton.jsx'

const PAGES = [
  { key:'story', title:'Hikaye', content:(<div className="space-y-3">
  ...
  { key:'goal', title:'Amaç', content:(<div className="space-y-3">
  ...
  { key:'rules', title:'Kurallar', content:(<div className="space-y-3">
  ...
  { key:'odds', title:'Olasılıklar', content:(<div className="space-y-3">
  ...
  { key:'monster', title:'KÇC Süreci', content:(<div className="space-y-3">
  ...
  { key:'methods', title:'Yöntemler & Taktikler', content:(<div className="space-y-3">
...
export default function ModalIntro({ open, onClose }) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm pointer-events-auto"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          onClick={onClose}                        // dışına tıkla kapansın
        >
          <motion.div
            className="relative w-full max-w-3xl bg-zinc-950 border border-zinc-800 rounded-3xl p-6 scanlines toxic-border pointer-events-auto"
            initial={{ scale: .9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: .9, opacity: 0 }}
            onClick={(e)=>e.stopPropagation()}     // içe tıklama bubbleda kapanmasın
          >
            <div className="absolute right-4 top-4 z-[110]">
              <button
                type="button"
                onClick={onClose}
                className="px-3 py-2 rounded-xl border border-zinc-700 bg-zinc-800/60 hover:bg-zinc-700 text-sm pointer-events-auto"
              >
                Kapat
              </button>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {idx > 0 && (
                  <button type="button" onClick={() => setIdx(i => i - 1)} className="px-3 py-2 rounded-xl border border-zinc-700 bg-zinc-800/60 hover:bg-zinc-700 text-sm">
                    ← Geri
                  </button>
                )}
                {idx < PAGES.length - 1 ? (
                  ...
                  <button type="button" onClick={onClose} className="px-3 py-2 rounded-xl border border-emerald-600 bg-emerald-700/60 hover:bg-emerald-600 text-sm">
                    Başla
                  </button>
                )}
              </div>
